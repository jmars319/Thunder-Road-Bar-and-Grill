...
- Shared PHP validator (`backend/utils/Validator.php`) applied inside every route (see `backend/routes/reservations.php`, `backend/routes/jobs.php`, etc.)
- Email format validation
- String length limits enforced
- File type validation (MIME type + magic bytes)

### File Upload Security ✅
- Magic byte + dimension validation via `backend/utils/FileValidator.php` (`verifyMagicBytes`, `getimagesize`)
- SVG sanitization handled by `FileValidator::sanitizeSvg`
